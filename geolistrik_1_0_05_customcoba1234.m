...
s1 = serial('COM2');				%posisi serial cek terlebih dahulu di device manager
s1.BaudRate=19200;
s1.StopBits=2;
s1.Timeout=10;
s1.Terminator='CR/LF';
...
s2 = serial('COM8','BaudRate',9600);		%posisi serial cek terlebih dahulu di device manager
fopen(s2);
pause(1);

%untuk membaca konfigurasi channel AMNB
nama_file_AMNB = input('contoh_configurasi_AMNB.csv: ', 's');
...
nama_file = input('hasilcontoh_configurasi_AMNB: ', 's');
